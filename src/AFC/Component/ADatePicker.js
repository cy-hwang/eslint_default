...
ADatePicker.prototype.init = function(context, evtListener)
{
	...
	if(this.$ele.attr('picker-type')) this.option.pickerType = this.$ele.attr('picker-type');
...
ADatePicker.prototype.setValue = function(date)
{
	...
	if(this.option.pickerType == 'yyyyMM') {

	} else if(afc.isPC) this.$ele.val(this.date.format('yyyy-MM-dd'));
